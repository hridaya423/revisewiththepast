...
import type { QuestionBankPart } from "@/lib/paper-maker/aqa-geography";
import type { RegionFigure, RegionPageLayout } from "@/lib/paper-maker/region-render";

export function isLocalGeometryEnabled() {
  return Boolean(process.env.PAPER_MAKER_LOCAL_GEOMETRY);
}
...
export function overlayQuestionBankWithLocalGeometry(parts: QuestionBankPart[]) {
  ...
  if (process.env.LOCAL_GEOMETRY_DEBUG) {
