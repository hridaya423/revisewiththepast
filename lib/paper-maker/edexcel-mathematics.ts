...
function loadMathematicsTaxonomy() {
  const filePath = resolve(process.cwd(), "config/edexcel-mathematics/taxonomy.json");
...
export function getEdexcelMathematicsTopicTree() {
  if (cachedTree) return cachedTree;

  const taxonomy = loadMathematicsTaxonomy();
...
export function expandEdexcelMathematicsTopicSelection(selectedNodeIds: string[]) {
  const index = cachedIndex ?? buildTopicIndex(getEdexcelMathematicsTopicTree());
  return Array.from(new Set(selectedNodeIds.flatMap((nodeId) => index.get(nodeId)?.leafTopicIds ?? [])));
}

export function buildEdexcelMathematicsTopicTreeWithCounts(units: QuestionUnit[]): TopicTreeNodeWithCounts[] {
  const tree = getEdexcelMathematicsTopicTree();
