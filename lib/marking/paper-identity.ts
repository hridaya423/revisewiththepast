...
export function detectPaperIdentityFromPages(
  ...
  const detectedYear = sessionYearMatch
    ? Number.parseInt(sessionYearMatch[1]?.length === 4 ? sessionYearMatch[1] : sessionYearMatch[2], 10)
    : null;
  const detectedSession = sessionYearMatch
    ? normalizeSession(sessionYearMatch[1]?.length === 4 ? sessionYearMatch[2] : sessionYearMatch[1])
    : null;
