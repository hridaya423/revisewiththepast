...
import { getPdfDocument, renderPdfPageToPng } from "@/lib/marking/pdfjs-server";
...
function pageHasQuestionStart(page: CachedPdfPage, questionNumber: string) {
  return page.lines.some((line) => detectLeftColumnQuestionNumber([line]) === questionNumber);
}

function detectOcrComputerScienceQuestionStart(page: CachedPdfPage, questionNumber: string) {
...
export function detectPageQuestionNumber(page: CachedPdfPage): string | null {
  ...
  const leftColumn = detectLeftColumnQuestionNumber(page.lines);
