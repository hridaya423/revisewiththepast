...
function loadEdexcelFrenchTaxonomy() {
  const filePath = resolve(process.cwd(), "config/edexcel-french/taxonomy.json");
...
export function getEdexcelFrenchTopicTree() {
  if (cachedTree) return cachedTree;

  const taxonomy = loadEdexcelFrenchTaxonomy();
...
export function expandEdexcelFrenchTopicSelection(selectedNodeIds: string[]) {
  const index = cachedIndex ?? buildTopicIndex(getEdexcelFrenchTopicTree());
  return Array.from(new Set(selectedNodeIds.flatMap((nodeId) => index.get(nodeId)?.leafTopicIds ?? [])));
}

export function buildEdexcelFrenchTopicTreeWithCounts(units: QuestionUnit[]): TopicTreeNodeWithCounts[] {
  const tree = getEdexcelFrenchTopicTree();
