...
function loadOcrComputerScienceTaxonomy() {
  const filePath = resolve(process.cwd(), "config/ocr-computer-science/taxonomy.json");
...
export function getOcrComputerScienceTopicTree() {
  if (cachedTree) return cachedTree;

  const taxonomy = loadOcrComputerScienceTaxonomy();
...
export function expandOcrComputerScienceTopicSelection(selectedNodeIds: string[]) {
  const index = cachedIndex ?? buildTopicIndex(getOcrComputerScienceTopicTree());
  return Array.from(new Set(selectedNodeIds.flatMap((nodeId) => index.get(nodeId)?.leafTopicIds ?? [])));
}

export function buildOcrComputerScienceTopicTreeWithCounts(units: QuestionUnit[]): TopicTreeNodeWithCounts[] {
  const tree = getOcrComputerScienceTopicTree();
